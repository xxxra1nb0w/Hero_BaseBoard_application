...
//    {
//        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_BACK;
//        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;
//    }
//}
...
//    static int auto_mode = 0; // 是否开启自瞄
//    if (!(gimbal_mode_set->last_press_r) && gimbal_mode_set->gimbal_rc_ctrl->mouse.press_r)
//    {
//        auto_mode = !auto_mode;
//    }
...
//    if (gimbal_behaviour == GIMBAL_TURN_ROUND) // 一键掉头时不更改模式
//    {
//        float delta = gimbal_mode_set->gimbal_yaw_motor.absolute_angle - gimbal_mode_set->gimbal_back_init_angle;
//        if (delta > 3.12f || (delta < -0.02f && delta > -3.16f))
//        {
